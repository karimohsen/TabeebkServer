 ...
 * @author Karim
 */
public class AdminAddClinic extends HttpServlet {
...
// Configure a repository (to ensure a secure temp location is used)
            ...
            try {
                ...
                while (iter.hasNext()) {
                    ...
                    if (item.isFormField()) {
                        if (item.getFieldName().equals("name")) {
                            clinic.setClinicName(item.getString());
                            branchName = item.getString();
                        } else if (item.getFieldName().equals("namear")) {
                            clinic.setClinicNameAr(item.getString("UTF-8").trim());
                            branchNameAr = item.getString("UTF-8").trim();
                        } else if (item.getFieldName().equals("clinicadd")) {
                            address = item.getString();
                        } else if (item.getFieldName().equals("clinicaddar")) {
                            addressAr = item.getString("UTF-8").trim();
                        ...
                        } else if (item.getFieldName().equals("Cities")) {
                        ...
                        if (!Name.equals("") && Name != null) {
                            ...
                            File file = new File(uploadFolder + "\\clinic");
                            if (!file.exists()) {
                                file.mkdir();
                            }
                            uploadFolder += "\\clinic";
                ...
                clinic.setDeleted(0);
                clinicId = ClinicDao.addClinic(clinic);
                if (tele1 != null) {
                    TelephoneDao.createTelephoneNumber(2, clinicId, tele1);
                }
                if (tele2 != null) {
                    TelephoneDao.createTelephoneNumber(2, clinicId, tele2);
                }
                MSPDao.addMsp(2, clinicId);
                BranchDao.addBranch(area, address, addressAr, latitude, longitude, branchName, branchNameAr, 2, clinicId, country, city);
