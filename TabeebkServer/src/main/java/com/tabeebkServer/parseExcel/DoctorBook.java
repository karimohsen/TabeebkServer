...
//                        mspDao.saveDoctor(doctorList.get(i));
    ...
    private TreeSet<Branche> createClinicBranch(Row clincRow, Clinic c, Workbook wb) {
        ...
        while (rowIterator.hasNext()) {
            ...
            if(c.getClinicName().trim().equals(clinicName.trim())){
            //if (clincRow.getRowNum() + 1 == clinicRowNum) {
                ...
                b = new Branche(city, area, mspType, areaNamesEn, areaNamesAr, fullAddressEn, fullAddressAr, c.getClinicId());
                branchs.add(b);
            }
        }
        return branchs;
    ...
    private TreeSet<Telephone> createClinicTelelphone(Row clinRow, Clinic c, Workbook ds) {
    ...
    private int saveDoctorSpeciality(Doctorspeciality ds) {
       
        return mspDao.saveDoctorSpeciality(ds);
    }
    ...
    private Doctorspeciality createDoctorSpeciality(Row row) {
        ...
        return mDoctorSpeciality = mspDao.getDoctorSpeciality(specialityNameEn.toLowerCase());
    }
